...
# elevando cada termo de psi_a ao quadrado e DEPOIS somando
...
def hamil_element(m,n): #calculo de Hmn para n,m inteiros arbitrarios, para um eletron confinada na caixa
	f1 = Lambda(x, sin(math.pi*m*x/L)*sin(math.pi*n*x/L)) #funçao lambda
	f2 = Lambda(x, x*sin(math.pi*m*x/L)*sin(math.pi*n*x/L))
	if m != n:
		if (m % 2 == 0 and n%2==0):
			H = 0
...
y3 = 0
i=0
while i<10: # nesse laço é somado os termos de Fourier (senos) com as coordenadas dos autovetores obtidos como coeficientes.
	y1 = y1 + (math.sqrt(2/L)*eigen_vectors()[0][i]*np.sin(math.pi*(i+1)*t/L))**2
	y2 = y2 + (math.sqrt(2/L)*eigen_vectors()[1][i]*np.sin(math.pi*(i+1)*t/L))**2
	y3 = y3 + (math.sqrt(2/L)*eigen_vectors()[2][i]*np.sin(math.pi*(i+1)*t/L))**2
	i+=1
